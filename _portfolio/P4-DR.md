...
<div style="overflow: auto; margin-bottom: 20px;"> <img src="{{ '/images/profile-jana-pavlikova.jpg' | prepend: site.baseurl }}" alt="Jana Pavlíková, Ph.D." style="float: left; margin-right: 15px; width: 200px; height: auto; border-radius: 50%;"> <p><b>Dr. Pavlíková</b> is a key collaborator whose expertise centers on the **computational methodology** that forms the core of this project. She is a principal developer of the **multi-scale computational framework** integrating **Molecular Dynamics (MD)**, **Dimensionality Reduction (DR)**, and **Clustering Algorithms (CA)** to model dynamic systems. Her specialized work validates the use of these techniques (like tICA/tSNE and machine learning-based clustering) to generate the small, highly representative **Clustered Ensembles (CEs)** necessary for accurate **NMR Chemical Shift (CS) predictions** in **Intrinsically Disordered Proteins (IDPs)**. Her contributions ensure the rigor of the ensemble selection protocol and the robust coupling of trajectory data with quantum calculations, specifically **Density Functional Theory (DFT)**, for achieving high-accuracy PTM-aware predictions.</p> </div>
